...
for coef in coefs:
    ...
    solutions =[]
    names = []
    for _, filename in enumerate(test_files):
    ## extract  feature
        for i in range(25):
            ...
            name = [ v.name[2:] for v in model.getVars(transformed = True)]
            ...
            solutions.append(pred_x.cpu().numpy())
            names.append(name)
    ...
    if output_type == "save":
        pickle.dump(obj_values, open(f'GenMIP/agents/diffusion_results/distribution_{instance}_{sampler_type}_{gradient_scale}_{coef}_objs.npy', 'wb'))
        pickle.dump(solutions, open(f'GenMIP/agents/diffusion_results/distribution_{instance}_{sampler_type}_{gradient_scale}_{coef}_sols.npy', 'wb'))
        pickle.dump(names, open(f'GenMIP/agents/diffusion_results/distribution_{instance}_{sampler_type}_{gradient_scale}_{coef}_names.npy', 'wb'))